...
namespace ClientJoinFix {
    internal static class Updater {
        ...
        public static void ApplyPendingIfAny(ManualLogSource log, string tag) {
        ...
        public static IEnumerator CheckForUpdate(ManualLogSource log, string releaseUrl, string currentVersion, string tag) {
            using (UnityWebRequest www = UnityWebRequest.Get(releaseUrl)) {
                ...
                using (UnityWebRequest dl = UnityWebRequest.Get(dllUrl)) {
                    ...
                    try {
                        File.WriteAllBytes(PendingPath, dl.downloadHandler.data);
